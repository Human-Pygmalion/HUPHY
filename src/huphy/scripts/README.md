...
`huphy-test` 와 `huphy-run` 이 루프가 끝난 뒤 한 번에 찍음. 텔레메트리를 켜지
않아도 됨 (`scripts/failures.py`).
...
| 버림 | 수신 스레드 큐가 차서 버린 프레임. 양다리만 해당 |
...
huphy-run --robot --policy balance --drain-ms 4
```

기본 2ms. 다 오면 즉시 빠져나가므로 정상 주기에는 이만큼 쓰지 않음.

| | |
|---|---|
| 늘리면 | 늦은 응답을 이번 주기에 받아 무응답·대기초과가 줄어듦 |
| 대신 | 모터 하나가 죽으면 매 주기 이만큼을 통째로 씀 → 밀림 |

양다리는 다리마다 따로 기다려서 **최악 두 배**임. 주기의 절반을 넘으면 집계에
경고가 찍힘. 주기(`--hz`)와 함께 바꿔 가며 실패 집계를 보는 용도임.
